# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  CELL 0 — AUTO-RUNNER HELPER                                            ║
# ╚══════════════════════════════════════════════════════════════════════════╝

from IPython.display import display, HTML

display(HTML("""
<div id="autorunner-status" style="
  padding: 8px 14px;
  background: #1a1a2e;
  ...
  margin: 4px 0;
">
  ▶ AutoRunner: initialising...
</div>
<script>
(async () => {
  const STATUS_ID = 'autorunner-status';
  const sleep = ms => new Promise(r => setTimeout(r, ms));

  function setStatus(msg, color) {
    const el = document.getElementById(STATUS_ID);
    if (el) { el.textContent = '▶ AutoRunner: ' + msg; if (color) el.style.borderLeftColor = color; }
  }

  function deepFind(root, test) {
    for (const el of root.querySelectorAll('*')) {
      if (test(el)) return el;
      if (el.shadowRoot) { const f = deepFind(el.shadowRoot, test); if (f) return f; }
    }
    return null;
  }

  setStatus('waiting 5s...', '#d29922');
  await sleep(5000);

  document.dispatchEvent(new KeyboardEvent('keydown', { key:'F9', code:'F9', ctrlKey:true, bubbles:true }));
  await sleep(2000);

  const runtimeBtn = deepFind(document, el => {
    const txt = (el.textContent||'').trim();
    return txt === 'Runtime' && el.tagName.match(/SPAN|A|BUTTON|DIV/i) && el.offsetParent;
  });
  if (runtimeBtn) {
    runtimeBtn.click(); await sleep(1000);
    const runAllBtn = deepFind(document, el =>
      (el.textContent||'').trim().toLowerCase() === 'run all' && el.offsetParent);
    if (runAllBtn) { runAllBtn.click(); setStatus('Run all triggered ✅', '#3fb950'); return; }
  }
  setStatus('please run manually (Ctrl+F9)', '#f85149');
})();
</script>
"""))

print("✅ AutoRunner cell executed.")
